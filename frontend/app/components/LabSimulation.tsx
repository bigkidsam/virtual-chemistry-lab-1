...
export interface WorldObject {
  ...
  flameOn?: boolean;
  chemical?: Chemical;
  liquidLevel?: number; // 0-1 for droppers/flasks
  color?: string;
}
...
const FLOOR_PAD = 140; // pixels from bottom (above platform)

const TOOL_EMOJI: Record<string, string> = {
  flask: "⚗️", beaker: "🧪", test_tube: "🧫", burner: "🔥",
  dropper: "💧", cylinder: "🥛", petri: "🫙", rod: "🥢",
  spatula: "🗡️",
  dropper_hcl: "💛", dropper_naoh: "🤍", dropper_cuso4: "💙",
  dropper_h2so4: "🩶", dropper_kcl: "🪨", dropper_ethanol: "💚",
};
...
export default function LabSimulation({
  ...
  // Mutable refs (avoid re-renders in animation loop)
  ...
  // Sync objectsRef when parent passes new objects (spawning)
  useEffect(() => {
  ...
  /* ------- Check dropper→slot interaction ------- */
  ...
  const animate = useCallback((time: number) => {
    ...
    for (const slot of slotsRef.current) {
      ...
      if (slot.chemicals.length === 0) {
        ...
        ctx.textAlign = "center";
        ctx.fillText(
          slot.chemicals.map((c) => c.name).join("+"),
          slot.x + SLOT_W / 2,
          slot.y + 16
        );
      }
    ...
    for (const obj of objectsRef.current) {
      ...
      // Emoji
      ctx.font = `${OBJECT_SIZE * 0.85}px serif`;
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText(obj.emoji, 0, 0);

      // Flame animation for burner
      if (obj.type === "burner" && obj.flameOn) {
        const flicker = Math.sin(time * 0.025) * 0.1;
        ctx.font = `${OBJECT_SIZE * (0.7 + flicker)}px serif`;
        ctx.globalAlpha = 0.9 + flicker * 0.1;
        ctx.fillText("🔥", 0, -OBJECT_SIZE * 0.9);
        ctx.globalAlpha = 1;
      }

      // Liquid level indicator for dropper
      if (obj.chemical && obj.liquidLevel != null && obj.liquidLevel > 0) {
        const barH = (OBJECT_SIZE * 0.5) * obj.liquidLevel;
        ctx.fillStyle = obj.color + "cc";
        roundedRect(ctx, -6, OBJECT_SIZE * 0.1, 12, barH, 3);
        ctx.fill();
      }
  ...
  }, [checkDropperSlot, checkReactions, spawnSmoke]);
  ...
  // Double-click to toggle burner flame
  ...
  // Method exposed via ref to spawn objects
  const [, forceUpdate] = useState(0);
  (LabSimulation as any)._spawnObject = (type: string, emoji: string, label: string, chemical?: Chemical) => {
    const { W, H } = sizeRef.current;
    const obj: WorldObject = {
      ...
      flameOn: false,
      chemical,
      liquidLevel: chemical ? 1 : undefined,
